...
<div class="gambar-atas">
	<img src="<?php echo base_url();?>assets/result-banner.jpg"/>
</div>
...
<div class="artikel-group">
	<?php
	if (isset($_POST['submit-cari']) && !empty($_POST['tx_cari'])) {
		# code...			
		$is_publish="1";
		$tag_id="Berita";
		$conn=new mysqli(HOST,USER,PASS,DB);
		if ($conn->connect_error) die("Connection failed: " . $conn->connect_error);
		$tx=$conn->escape_string($_POST['tx_cari']);
		$sql="SELECT id, tag_id, judul, SUBSTR(isi,1,200), foto, users_id, tgl, url_friendly FROM artikel WHERE is_publish=? AND (isi LIKE ? OR judul LIKE ?) ORDER BY tgl DESC;";
		$stmt=$conn->prepare($sql);
		$tx="%".$tx."%";
		$stmt->bind_param('sss',$is_publish,$tx,$tx);
		$stmt->execute();
